...
```{r, solution_01d}
# R Console warns that 'size' for aes has been deprecated for ggplot 3.4.0, and to use 'linewidth' instead. I tried it and there seems to be no difference in output versus using 'size' except the warning. Yay! -Matt

iris %>% 
  ggplot(mapping = aes(x = Sepal.Length)) +
  geom_histogram(color = 'navyblue', fill = 'gold', bins = 6, linewidth = 1.55)
```
...
iris %>% 
  ggplot(mapping = aes(x = Sepal.Length)) +
  geom_histogram(color = 'navyblue', fill = 'gold', bins = 6, linewidth = 1.55, alpha = 0.5)
```
...
For this boxplot, we can see that its distribution skews right. Suggesting that diamond prices by color are more varied above the median. It has been awhile since STAT 1000, but I don't think this amount of skew is ideal. Especially with D, E, and F, there are a lot of extreme outliers, datapoints greater than (Q3 + 1.5 * IQR), in the data.

CONCLUSION: I do not believe that the color of a diamond by itself is a sufficient indicator of price.
...
diamonds %>% 
  ggplot(mapping = aes(x = color, y = price)) +
  geom_boxplot(mapping = aes(fill = carat > median(carat))) + facet_grid(clarity ~ cut) + theme(legend.position = 'top')
```
...
Looking at these two boxplots side-by-side, the first thing we can see is that the distribution has changed. However, the median prices seem more in line with what little I know about diamonds. D, E, and F (colorless) diamonds typically command a higher price on average, and the boxplot from 2b did not reflect that. Also, we see far fewer outliers in 2f, which is a good sign, and the majority of outliers in 2f's boxplot are when "carat > median(carat)" evaluated false. This suggests that the large variance in price we saw in 2b was not related to the color rating.

CONCLUSION: By using the "4 C's" (2f) instead of just any 1 C (2b), we get a much more accurate view of diamond prices. 
...
The confidence interval does not contain zero, therefore 'displ' (x) IS statistically significant.
...
All variables are statistically significant. There are four coefficients including the intercept.
...
$$
\begin{bmatrix} \mathbf{1} & \mathbf{2} & \mathbf{3}\\
\mathbf{4} & \mathbf{5} & \mathbf{6}\\
\mathbf{7} & \mathbf{8} & \mathbf{9} \end{bmatrix} \begin{bmatrix} \mathbf{x}\\ \mathbf{y}\\ \mathbf{z}\\ \end{bmatrix} = \begin{bmatrix} \mathbf{6}\\ \mathbf{15}\\ \mathbf{24}\\ \end{bmatrix}
$$
