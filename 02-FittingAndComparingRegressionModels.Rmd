...
The relationship appears to be non-linear (flat) or maybe a very weak negative correlation.
...
fit_lm_1 <- lm(y ~ 1, data = df_low_high)
```
...
rmse_df = data.frame(fit = c('linear', 'quadratic', 'cubic', '4th degree', '5th degree', '6th degree', '7th degree', '8th degree', '9th degree'), RMSE = c(rmse1, rmse2, rmse3, rmse4, rmse5, rmse6, rmse7, rmse8, rmse9))
...
```{r, solution_01d}
rsqu1 <- modelr::rsquare(fit_lm_1, df_low_high)
rsqu2 <- modelr::rsquare(fit_lm_2, df_low_high)
rsqu3 <- modelr::rsquare(fit_lm_3, df_low_high)
rsqu4 <- modelr::rsquare(fit_lm_4, df_low_high)
rsqu5 <- modelr::rsquare(fit_lm_5, df_low_high)
rsqu6 <- modelr::rsquare(fit_lm_6, df_low_high)
rsqu7 <- modelr::rsquare(fit_lm_7, df_low_high)
rsqu8 <- modelr::rsquare(fit_lm_8, df_low_high)
rsqu9 <- modelr::rsquare(fit_lm_9, df_low_high)

rsqu_df = data.frame(fit = c('linear', 'quadratic', 'cubic', '4th degree', '5th degree', '6th degree', '7th degree', '8th degree', '9th degree'), Rsquare = c(rsqu1, rsqu2, rsqu3, rsqu4, rsqu5, rsqu6, rsqu7, rsqu8, rsqu9))
...
The model that uses the 9th degree polynomial (Model 9) seems to be the best of our models, and both RMSE and R-squared would agree. Model 9 has both the lowest RMSE and highest R-squared. This is in line with what we learned during lecture, that models with greater complexity (more coefficients) have better performance.
...
There seems to be only one coefficient where at least 2 different models are similar, I(x^3) for fit_lm_6 and fit_lm_9 though the margin for fit_lm_9 is much larger than fit_lm_6  
...
```{r, solution_02d}
ggplot(data = df_viz_1, x = x) + 
  geom_line(mapping = aes(x = x, y = pred_trend)) #+
...
# As per the instructions, I had originally ordered "geom_point" before "geom_line" but I noticed the line obscured some of the red points, so I reversed the order for visibility. Will e-mail about it, but leaving this comment here until I do. -Matt

ggplot(data = df_viz_1, mapping = aes(x = x)) + 
  geom_line(mapping = aes(y = pred_trend, linewidth = 1)) +
...
```{r, solution_02h}
ggplot(data = df_viz_all, mapping = aes(x = x)) + 
...
Previously chose Model 9 (9th degree polynomial) based on the RMSE and R-squared from Problem 1. Seems consistent.
...
```{r, solution_03b}
ggplot() + 
  geom_point(data = df_train, mapping = aes(x = x, y = y, color = 'Training')) +
  geom_point(data = df_holdout, mapping = aes(x = x, y = y, color = 'Holdout'), shape = 0) +
  labs(color = "Legend") +
...
### linear relationship (degree 1)
fit_train_1 <- lm(y ~ 1, data = df_train)
```
...
```{r, solution_03d}
t_rmse1 <- modelr::rmse(fit_train_1, df_holdout)
t_rmse2 <- modelr::rmse(fit_train_2, df_holdout)
t_rmse3 <- modelr::rmse(fit_train_3, df_holdout)
t_rmse4 <- modelr::rmse(fit_train_4, df_holdout)
t_rmse5 <- modelr::rmse(fit_train_5, df_holdout)
t_rmse6 <- modelr::rmse(fit_train_6, df_holdout)
t_rmse7 <- modelr::rmse(fit_train_7, df_holdout)
t_rmse8 <- modelr::rmse(fit_train_8, df_holdout)
t_rmse9 <- modelr::rmse(fit_train_9, df_holdout)

t_rmse_df = data.frame(fit = c('linear', 'quadratic', 'cubic', '4th degree', '5th degree', '6th degree', '7th degree', '8th degree', '9th degree'), RMSE = c(t_rmse1, t_rmse2, t_rmse3, t_rmse4, t_rmse5, t_rmse6, t_rmse7, t_rmse8, t_rmse9))
...
Best according to hold-out is the quadratic model (lowest RMSE). Not the same model that was IDed by training set performance. That model ranked the worst with the highest RMSE by a large margin.
...
```{r, solution_03f}
#coefplot::multiplot(fit_lm_2, fit_train_2, fit_lm_3, fit_train_3)
coefplot::multiplot(fit_lm_9, fit_train_9)
```

What do you think?  

It looks like the amount of uncertainty is greater for the coefficients of fit_train_9, not all values of the coefficients are similar, but most are close.
...
Like 3f, the amount of uncertainty of coefficients is greater for fit_train_2. Also, it looks like there is greater variation in values than 3f.
...
Each trained and tested FIVE times.
...
From the possible choices RMSE seemed like the best choice for a regression problem as that seems to have been our go to metric in both lecture and this homework set.
...
```{r, solution_04f}
coefplot::multiplot(mod_lowhigh_1, mod_lowhigh_2, mod_lowhigh_3)
```

What do you think?  

Mostly just the intercept (all three models), but 2 and 3 are close at the I(x^2) coefficient.

The amount of uncertainty is fairly consistent across all three models even though the value of their coefficients isn't exactly close.
...
It looks parabolic compared to 1a!
...
Our resampling scheme did not choose the same model for the two noise cases. For the high noise set, the cubic model (3rd degree polynomial) performed the best, but for the low noise set our 6th degree polynomial model performed the best by quite a bit.

The 7th, 8th, and 9th degree polynomial models performed terribly in both noise cases.
...
The data points are "tighter", closer to parabolic, and there looks to be a stronger and positive correlation compared to 1a.
...
5th degree polynomial model (fit_05).
...
Nope. Once again our resampling scheme has picked a different model. However, this time the scheme identified the 7th and 8th degree polynomial models as performing better than they did for the other two sets. The 9th degree polynomial model was the worst across all three.
...
I think that a larger sample size is preferable, but a lower noise level seems to have made more of a difference than sample size.
