...
Include as many equation blocks and as much
discussion text as you feel are necessary.
...
$$
\frac{2\mu_0}{2} = \mu_0 = 11
$$
Solve for $\tau_0$

$$
\mu_0 - 2\tau_0 = 11 - 2\tau_0 = 10
$$
$$
2\tau_0 = 1
$$
...
eval_logpost <- function(unknown_1, unknown_2, logpost_func, logpost_info)
{
  ...
  log_posterior <- logpost_func(unknowns, logpost_info)
...
param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = mean(param_grid$mu), xsd = sd(param_grid$sigma)),
...
$$
\frac{d^2}{d\mu^2} \log p \left( \mu \mid \mathbf{x}, \sigma \right)
$$

Log-likelihood:
$$
\frac{d^2}{d\mu^2} \sum_{n=1}^N \left( \log \mathbf{normal} \left( x_n \mid \mu, \sigma \right) \right)
$$

Log-prior:
$$
\frac{d^2}{d\mu^2} \log \mathbf{normal} \left( \mu \mid \mu_0, \tau_0 \right)
$$

2nd derivative of log posterior:
$$
\frac{d^2}{d\mu^2} \log p \left( \mu \mid \mathbf{x}, \sigma \right) = - \frac{N}{\sigma^2} - \frac{1}{\tau_0}
$$
...
It corresponds to the value that maximizes log-posterior density. Hence, the most likely estimate with the given data.
...
Posterior Mode:
$$
\mathbf{m_N} = \frac{\sum_{n=1}^N x_n + \tau_0\mu_0}{N + \tau_0}
$$

Posterior SD:
$$
\mathbf{s_N} = \frac{1}{\sqrt{\frac{N}{\sigma^2} + \frac{1}{\tau_0}}}
$$
...
With known variance, the distribution is a conjugate prior for the normal likelihood. Hence, the posterior distribution is also a normal distribution.
...
Simplification! The natural log transformation grants us a linear relationship between mu and varphi which makes our job easier when using the Laplace Approximation. This transformation also grants us an unbounded range for more flexible modeling.
...
$$
p \left( \mu, \varphi, \mathbf{x} \right) \propto p \left( \mu \right) \times p \left( \varphi \right) \times \left| \frac{d}{d\sigma} g^{-1} \left( \varphi \right) \right| \times \prod_{n=1}^N p \left( x_n \mid \mu, g^{-1} \left( \varphi \right) \right)
$$
...
$$
\sigma = g^{-1} \left( \varphi \right) = e^\varphi
$$

Write the log of the derivative adjustment in an equation block.  

$$
\log \left( \frac{e^\varphi}{\sigma} \right)
$$
...
my_cv_logpost <- function(unknowns, my_info)
{
  ...
  log_deriv_adjust <- lik_varphi - log(lik_sigma)
...
How am I off by exactly one on this test? The other tests for this and the my_logpost were spot on. Geez. -Matt
...
varphi_grid_upr <- log(qexp(0.99, rate = hw06_info$sigma_rate))
```
...
log_post_cv_result <- purrr::map2_dbl(cv_param_grid$mu, cv_param_grid$varphi, eval_logpost, logpost_fun = my_cv_logpost, logpost_info = hw06_info)
```
...
cv_param_grid %>% 
  ...
  geom_point(data = tibble::tibble(xbar = mean(cv_param_grid$mu), log_xsd = sd(cv_param_grid$varphi)),
...
map_res_01 <- optim(par = init_guess_01,
                    ...
                    method = "BFGS",
                    hessian = TRUE,
                    control = list(maxit = 1001, fnscale = -1))
```
...
map_res_02
```
There are some slight differences in the values produced from the guesses. The Hessian matrices are not identical, but they are close. Nothing too major aside from the "count" fields in the results.
...
laplace_result_01 <- my_laplace(init_guess_01, my_cv_logpost, my_info = hw06_info)

print("Posterior Mean:")
laplace_result_01$mode[1]
print("Posterior SD:")
sqrt(laplace_result_01$mode[2])
print("Converged:")
laplace_result_01$converge
print("Correlation coefficient between mu and varphi:")
-(laplace_result_01$var_matrix[1,2] / sqrt(laplace_result_01$var_matrix[1,1] * laplace_result_01$var_matrix[2,2]))
```
```{r, solution_04e_2, eval=TRUE}
laplace_result_02 <- my_laplace(init_guess_02, my_cv_logpost, my_info = hw06_info)

print("Posterior Mean:")
laplace_result_02$mode[1]
print("Posterior SD:")
sqrt(laplace_result_02$mode[2])
print("Converged:")
laplace_result_02$converge
print("Correlation coefficient between mu and varphi:")
-(laplace_result_02$var_matrix[1,2] / sqrt(laplace_result_02$var_matrix[1,1] * laplace_result_02$var_matrix[2,2]))
```
...
There is a 41.24% chance that our unknown mean is less than the sample average.

```{r, 05a_solution}
posterior_mu <- laplace_result_01$mode[1]

posterior_varphi <- laplace_result_01$mode[2]

sample_avg <- mean(hw06_df$x)

pnorm(sample_avg, posterior_mu, exp(posterior_varphi))
```
...
I got an error about the var_matrix not being positive definite in the following question so I'm guessing I screwed up royally somewhere way back, possibly with the my_laplace function. Had a ton of midterms to study for this week. Wish I had given myself more time for this assignment. May God have mercy on my soul. :(
...
```
Inverse transformation of Varphi mean shows no difference with the Sigma mean. As shown in the following code chonk. Maybe I didn't screw up as badly as I thought? 

```{r, solution_05c_3}
mean(post_samples$sigma) - mean(exp(post_samples$varphi))
```
...
Mu:
```{r, 05d_solution_1}
ggplot(post_samples, aes(x = mu)) +
  geom_histogram(binwidth = 55/55, fill = "blue", color = "black") +
  labs(title = "Posterior mu", x = expression(mu), y = "Frequency") +
  theme_minimal()
```

Sigma:
```{r, 05d_solution_2}
ggplot(post_samples, aes(x = sigma)) +
  geom_histogram(binwidth = 55/55, fill = "red", color = "black") +
  labs(title = "Posterior sigma", x = expression(sigma), y = "Frequency") +
  theme_minimal()
```

I'd say the distribution for Sigma is Gaussian-like, but not perfect. The distribution for Mu is much more Gaussian
...
There is a 4.95% chance that the noise is greater than 4 units.

```{r, 05e_solution}
1 - pnorm(4, mean = mean(post_samples$sigma), sd = sd(post_samples$sigma))
```