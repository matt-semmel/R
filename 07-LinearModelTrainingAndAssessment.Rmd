...
```{r, 01a_solution}
quad_data_big_tib <- quad_data_big %>% rowid_to_column("rowid")

ggplot(quad_data_big_tib, aes(x = x, y = y)) +
  geom_point(aes(color = rowid > 5), size = 3) +
  labs(color = "First 5 Rows") +
  theme_minimal()
```
...
```{r, 01c_solution_a}
print("Rows / Columns of Xmat_quad_small:")
dim(Xmat_quad_small)
print("Rows / Columns of Xmat_quad_big:")
dim(Xmat_quad_big)
```
...
The first column contains the intercept. All values are "1".
...
```{r, 01e_solution}
print("Rows / Columns of covmat_quad_small_noprior:")
dim(covmat_quad_small_noprior)
print("Rows / Columns of covmat_quad_big_noprior:")
...
```{r, 01f_solution}
print("Posterior SD of quadratic feature of comvat_quad_small_noprior: ")
sqrt(covmat_quad_small_noprior[3, 3])
print("Posterior SD of quadratic feature of comvat_quad_big_noprior: ")
sqrt(covmat_quad_big_noprior[3, 3])
```
...
The values would most certainly change if $\sigma$ was equal to 5 instead of 1. As mentioned earlier in Problem 1, $\sigma$ is a scaling factor in our calculations. A larger $\sigma$ would imply more uncertainty and would result in a wider distribution for our coefficients.
...
```{r, 01h_solution}
posterior_se_small <- sqrt(covmat_quad_small_noprior[3, 3])
posterior_se_big <- sqrt(covmat_quad_big_noprior[3, 3])

margin_of_error_small <- qnorm(0.975) * posterior_se_small
margin_of_error_big <- qnorm(0.975) * posterior_se_big

confidence_interval_small <- c(postmeans_quad_small_noprior[3] - margin_of_error_small, postmeans_quad_small_noprior[3] + margin_of_error_small)
confidence_interval_big <- c(postmeans_quad_big_noprior[3] - margin_of_error_big, postmeans_quad_big_noprior[3] + margin_of_error_big)

print("Small 95% Uncertainty Interval: ")
confidence_interval_small
print("Big 95% Uncertainty Interval: ")
confidence_interval_big
```

What do you think?  

The SMALL data's uncertainty interval does not include zero. Assuming $\sigma = 1$ then the quadratic feature could be considered statistically significant.

The BIG data's uncertainty interval does not include zero. Assuming $\sigma = 1$ then the quadratic feature could be considered statistically significant.
...
```{r, display_for_2a}
print("Informative Prior Covariance: ")
B0_inform
print("Vague Prior Covariance: ")
B0_vague
```
...
covmat_quad_small_inform <- solve(solve(B0_inform) + (t(Xmat_quad_small[1, , drop = FALSE]) %*% Xmat_quad_small[1, , drop = FALSE]) / 1.4^2)
...
covmat_quad_big_inform <- solve(solve(B0_inform) + (t(Xmat_quad_big[1, , drop = FALSE]) %*% Xmat_quad_big[1, , drop = FALSE]) / 1.4^2)
...
covmat_quad_small_vague <- solve(solve(B0_vague) + (t(Xmat_quad_small[1, ,drop=FALSE]) %*% Xmat_quad_small[1, ,drop=FALSE]) / 30^2)
...
covmat_quad_big_vague <- solve(solve(B0_vague) + (t(Xmat_quad_big[1, ,drop=FALSE]) %*% Xmat_quad_big[1, ,drop=FALSE]) / 30^2)
...
```{r, 02d_solution_a}
Small_Inform2D <- sqrt(diag(1.4^2 * solve(t(Xmat_quad_small[1:3, , drop = FALSE]) %*% Xmat_quad_small[1:3, , drop = FALSE])))

Small_Inform2D[3]
```

```{r, 02d_solution_b}
Big_Inform2D <- sqrt(diag(1.4^2 * solve(t(Xmat_quad_big[1:3, , drop = FALSE]) %*% Xmat_quad_big[1:3, , drop = FALSE])))

Big_Inform2D[3]
```

Compared to the results from 1f they are not similar.
...
```{r, 02e_solution_a}
Small_Vague2D <- sqrt(diag(30^2 * solve(t(Xmat_quad_small[1:3, , drop = FALSE]) %*% Xmat_quad_small[1:3, , drop = FALSE])))

sqrt(1.4^2 * covmat_quad_small_noprior[3, 3])

Small_Vague2D[3]
```

```{r, 02e_solution_b}
Big_Vague2D <- sqrt(diag(30^2 * solve(t(Xmat_quad_big[1:3, , drop = FALSE]) %*% Xmat_quad_big[1:3, , drop = FALSE])))

Big_Vague2D[3]
```

Compared to the results from 1f they are even less similar.
...
Like many of the problems we have worked on in these homework sets, the prior can affect ranges of values. An informative prior tends to give us a tighter range that is more likely to be closer to the truth, while a vague may show a pattern that's close to the truth but the values are unlikely to be truthful.
...
info_quad <- list(
  yobs = as.vector(quad_data_big$y),
  design_matrix = model.matrix(y ~ x + I(x^2), data = quad_data_big),
  mu_beta = 0,
  tau_beta = 3,
  sigma_rate = 1
)
...
```{r, solution_03b_b}
test_values_1 <- c(-1, -1, -1, -1)
log_posterior_1 <- lm_logpost(test_values_1, info_quad)
...
```{r, solution_03b_c}
test_values_2 <- c(1, 1, 1, 1)
log_posterior_2 <- lm_logpost(test_values_2, info_quad)
...
laplace_quad <- my_laplace(start_guess = c(0, 0, 0, 0), logpost_func = lm_logpost, info_quad)
...
print(laplace_quad$mode)
print("Posterior Standard Deviations:")
print(sqrt(diag(laplace_quad$var_matrix)))
```

Should you be worried about the initial guess impacted results?  

Possibly. We may see convergence at different modes depending on where we start our guess. Choosing initial guesses close to the post mode would be ideal, but that is not always possible.
...
generate_lm_post_samples <- function(mvn_result, length_beta, num_samples)
{
  MASS::mvrnorm(n = num_samples,
    ...
    mutate(sigma = exp(mvn_result$mode[length_beta + 1]))
}
```
...
set.seed(87123)
post_samples_quad <- generate_lm_post_samples(laplace_quad, length_beta = length(laplace_quad$mode) - 1, num_samples = 2500)
```
...
```{r, solution_03d_histogram}
ggplot(post_samples_quad, aes(x = beta_02)) +
  geom_histogram(bins = 55, fill = "blue", color = "black", alpha = 0.7) +
  labs(x = expression(beta[2]), y = "Frequency")
```

```{r, solution_03e_probs}
print("95% Posterior Interval for beta_02:")
quantile(post_samples_quad$beta_02, c(0.025, 0.975))

print("Probability that beta_02 is greater than zero:")
...
The rows are prediction points of the design matrix, same number of rows. The columns are the posterior samples.
...
quad_data_big_y <- quad_data_big$y

### error of the first posterior sample
error_quad_post_01 <- quad_data_big_y - post_trend_samples_quad[,1]

### error of the second posterior sample
error_quad_post_02 <- quad_data_big_y - post_trend_samples_quad[,2]

### error of the third posterior sample
error_quad_post_03 <- quad_data_big_y - post_trend_samples_quad[,3]
```
...
The length should be the same as the number of predictions (rows) in our design matrix which will have the same number of observations (rows) as our BIG dataset, for our homework that would be 30. Let's double check!
...
With the MAE, we are averaging over the absolute value of differences between predictions and observations. Sometimes the differences could be negative numbers which would throw off the mean value, so it's important that we work with absolute values here.

Our three MAE values are different, which should be expected because we used three different posterior samples in calculating the error vectors we are getting these MAE values from.
...
absError_quad_mat <- abs(post_trend_samples_quad - quad_data_big_y)
...
The posterior samples are associated with the columns in our matrices. colMeans() would be the appropriate choice for this question.
...
### your code here
print("Column 1, 4d and 4f:")
mae_quad_post_01
MAE_quad[1]

print("Column 2, 4d and 4f:")
mae_quad_post_02
MAE_quad[2]

print("Column 3, 4d and 4f:")
mae_quad_post_03
MAE_quad[3]
```

Woohoo! The first three columns are consistent indeed! Probably safe to assume that all values of the MAE vector are correct. Probably.
...
Because we're basing it on multiple posterior samples. Since each sample is based on different parameters we get a different MAE for each. We can get a sense of the distribution of the MAE, but we won't get a singular value.
...
```{r solution_05a}
ggplot() +
  geom_point(data = train_df, aes(x = x, y = y), color = "blue", alpha = 0.5) +
  geom_point(data = test_df, aes(x = x, y = y), color = "red", alpha = 0.5) +
  labs(x = "Input (x)", y = "Response (y)") +
...
{
  # Create the basis for the 'x' variable in the training set
  train_basis <- splines::ns(train_data$x, df = J)

  # Extract interior and boundary knots
  ...
  train_matrix <- model.matrix(~ train_basis, data = train_data)
  
  # Create the test design matrix using the same knots as the training matrix
  test_matrix <- model.matrix(~ splines::ns(test_data$x, df = J, knots = interior_knots, Boundary.knots = boundary_knots), data = test_data)
  
  # Return a tibble with design matrices for training and testing
  tibble::tibble(
...
The design matrices contain 150 rows. The test matrices contain 50 rows.
...
manage_spline_fit <- function(Xtrain, logpost_func, my_settings)
{
  ...
  init_varphi <- log(rexp(my_settings$sigma_rate))
...
all_spline_mae_results %>%
  ...
  stat_summary(geom = 'line', fun = 'median', aes(color = dataset)) +
  scale_color_brewer(palette = "Set1")
```

Which degrees-of-freedom overfit?  

It appears that degrees-of-freedom greater than 5 or 7 will overfit the model.
...
tibble::tibble(
  ...
  labs(x = "Degrees of Freedom (J)", y = "Model Weight")
```

Which model is the best?  

The model where J = 7 appears to be the best, with J = 9 in a very distant 2nd.
...
Conclusions are not the same. Close, but not the same.

The single train looks at model performance on a specific split and gives us an idea how well that specific model generalizes to the test set used. The evidence-based considers models' relative likelihood based on evidence, and weighing all models to give us an idea of how models will perform on average across different sets.

In short, I would not expect us to get the same conclusions from those two assessments.
