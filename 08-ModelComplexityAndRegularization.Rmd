...
The scatterplot for x2 and y looks less, well, scattered, than the plot for x1 and y. Could be a trend in x2.
...
coefplot(mod02)
```
There are 4 including the intercept, the three features that are not the intercept are statistically significant.
...
coefplot(mod03)
```
9 features including the intercept. Only the intercept and I(x2^2) are statistically significant.
...
coefplot(mod04)
```
THere are 25 features. That is more than I can count on my fingers and toes. Only the intercept is statistically significant.
...
```{r, solution_01h}
model_metrics <- data.frame(Model = c("mod01", "mod02", "mod03", "mod04", "mod05", "mod06"))

model_metrics$R_squared <- broom::glance(mod01)$r.squared
model_metrics$AIC <- AIC(mod01)
model_metrics$BIC <- BIC(mod01)

model_metrics$R_squared[2] <- broom::glance(mod02)$r.squared
model_metrics$AIC[2] <- AIC(mod02)
model_metrics$BIC[2] <- BIC(mod02)

model_metrics$R_squared[3] <- broom::glance(mod03)$r.squared
model_metrics$AIC[3] <- AIC(mod03)
model_metrics$BIC[3] <- BIC(mod03)

model_metrics$R_squared[4] <- broom::glance(mod04)$r.squared
model_metrics$AIC[4] <- AIC(mod04)
model_metrics$BIC[4] <- BIC(mod04)

model_metrics$R_squared[5] <- broom::glance(mod05)$r.squared
model_metrics$AIC[5] <- AIC(mod05)
model_metrics$BIC[5] <- BIC(mod05)

model_metrics$R_squared[6] <- broom::glance(mod06)$r.squared
model_metrics$AIC[6] <- AIC(mod06)
model_metrics$BIC[6] <- BIC(mod06)

model_metrics
```
In terms of R-Squared, the best model is mod06. In terms of AIC, mod03 is the best model. In terms of BIC, mod03 is the best model. I'll go with mod03 overall.
...
```{r, solution_02c_3}
ggplot(pred_lm_03, aes(x = x1, y = pred)) +
  geom_ribbon(aes(ymin = pred_lwr, ymax = pred_upr), fill = "orange") +
  geom_ribbon(aes(ymin = ci_lwr, ymax = ci_upr), fill = "grey") +
  geom_line() +
  facet_wrap(~x2, scales = "free_x") +
...
```{r, solution_02c_4}
ggplot(pred_lm_04, aes(x = x1, y = pred)) +
  geom_ribbon(aes(ymin = pred_lwr, ymax = pred_upr), fill = "orange") +
  geom_ribbon(aes(ymin = ci_lwr, ymax = ci_upr), fill = "grey") +
  geom_line() +
  facet_wrap(~x2, scales = "free_x") +
...
```{r, solution_02c_5}
ggplot(pred_lm_05, aes(x = x1, y = pred)) +
  geom_ribbon(aes(ymin = pred_lwr, ymax = pred_upr), fill = "orange") +
  geom_ribbon(aes(ymin = ci_lwr, ymax = ci_upr), fill = "grey") +
  geom_line() +
  facet_wrap(~x2, scales = "free_x") +
...
```{r, solution_02c_6}
ggplot(pred_lm_06, aes(x = x1, y = pred)) +
  geom_ribbon(aes(ymin = pred_lwr, ymax = pred_upr), fill = "orange") +
  geom_ribbon(aes(ymin = ci_lwr, ymax = ci_upr), fill = "grey") +
  geom_line() +
  facet_wrap(~x2, scales = "free_x") +
...
I think they are consistent with the performance rankings. The defining characteristic of the models considered to be the worse by AIC/BIC is complexity. We can see in the visualizations that the more complex models have more fluctuations and noise.
...
lm_logpost <- function(unknowns, my_info)
{
  ...
  log_lik <- sum(dnorm(my_info$yobs, mu, lik_sigma, log = TRUE))
  ...
  # sum together
  as.numeric(log_lik + log_prior + log_derive_adjust)
}
```
...
```{r, solution_03c}
laplace_03_weak <- my_laplace(c(rep(0, ncol(X03)), 0), lm_logpost, my_info = info_03_weak)

laplace_03_weak$converge
```

```{r, solution_03c_2}
laplace_06_weak <- my_laplace(c(rep(0, ncol(X06)), 0), lm_logpost, my_info = info_06_weak)
...
```{r, solution_03d_a}
post_means_03 <- laplace_03_weak$mode[1:(length(laplace_03_weak$mode) - 1)]
post_sds_03 <- sqrt(diag(laplace_03_weak$var_matrix))[1:(length(laplace_03_weak$mode) - 1)]
feature_names_03 <- colnames(X03)[1:(length(laplace_03_weak$mode) - 1)]

viz_post_coefs(post_means_03, post_sds_03, feature_names_03)
```


```{r, solution_03d_b}
post_means_06 <- laplace_06_weak$mode[1:(length(laplace_06_weak$mode) - 1)]
post_sds_06 <- sqrt(diag(laplace_06_weak$var_matrix))[1:(length(laplace_06_weak$mode) - 1)]
feature_names_06 <- colnames(X06)[1:(length(laplace_06_weak$mode) - 1)]

viz_post_coefs(post_means_06, post_sds_06, feature_names_06)
```
...
Bayes factor greater than 1, Model 3 is better.
...
info_03_strong <- list(
  yobs = df$y,
  design_matrix = model.matrix(mod03),
  mu_beta = 0,
  tau_beta = 1,
  sigma_rate = 1 
)
...
info_06_strong <- list(
  yobs = df$y,
  design_matrix = model.matrix(mod06),
  mu_beta = 0,
  tau_beta = 1,
  sigma_rate = 1 
)
...
```{r, solution_03f_b}
laplace_03_strong <- my_laplace(rep(0, ncol(info_03_strong$design_matrix) + 1), 
                                lm_logpost, 
                                my_info = info_03_strong)

laplace_06_strong <- my_laplace(rep(0, ncol(info_06_strong$design_matrix) + 1), 
                                lm_logpost, 
                                my_info = info_06_strong)
```
...
```{r, solution_03g}
post_means_03_strong <- laplace_03_strong$mode[1:(length(laplace_03_strong$mode) - 1)]
post_sds_03_strong <- sqrt(diag(laplace_03_strong$var_matrix))[1:(length(laplace_03_strong$mode) - 1)]
feature_names_03_strong <- colnames(info_03_strong$design_matrix)[1:(length(laplace_03_strong$mode) - 1)]

viz_post_coefs(post_means_03_strong, post_sds_03_strong, feature_names_03_strong)
```

```{r, solution_03g_mod06}
post_means_06_strong <- laplace_06_strong$mode[1:(length(laplace_06_strong$mode) - 1)]
post_sds_06_strong <- sqrt(diag(laplace_06_strong$var_matrix))[1:(length(laplace_06_strong$mode) - 1)]
feature_names_06_strong <- colnames(info_06_strong$design_matrix)[1:(length(laplace_06_strong$mode) - 1)]

viz_post_coefs(post_means_06_strong, post_sds_06_strong, feature_names_06_strong)
```
...
```{r, solution_03h_b}
laplace_03_very_strong <- my_laplace(rep(0, ncol(info_03_very_strong$design_matrix) + 1), 
                                      lm_logpost, 
                                      my_info = info_03_very_strong)

laplace_06_very_strong <- my_laplace(rep(0, ncol(info_06_very_strong$design_matrix) + 1), 
                                      lm_logpost, 
                                      my_info = info_06_very_strong)
```
...
```{r, solution_03i}
post_means_03_very_strong <- laplace_03_very_strong$mode[1:(length(laplace_03_very_strong$mode) - 1)]
post_sds_03_very_strong <- sqrt(diag(laplace_03_very_strong$var_matrix))[1:(length(laplace_03_very_strong$mode) - 1)]
feature_names_03_very_strong <- colnames(info_03_very_strong$design_matrix)[1:(length(laplace_03_very_strong$mode) - 1)]

viz_post_coefs(post_means_03_very_strong, post_sds_03_very_strong, feature_names_03_very_strong)
```

```{r, solution_03i_mod06}
post_means_06_very_strong <- laplace_06_very_strong$mode[1:(length(laplace_06_very_strong$mode) - 1)]
post_sds_06_very_strong <- sqrt(diag(laplace_06_very_strong$var_matrix))[1:(length(laplace_06_very_strong$mode) - 1)]
feature_names_06_very_strong <- colnames(info_06_very_strong$design_matrix)[1:(length(laplace_06_very_strong$mode) - 1)]

viz_post_coefs(post_means_06_very_strong, post_sds_06_very_strong, feature_names_06_very_strong)
```
...
When the prior SD is weak it has minimal influence on the posterior distribution and said distribution tends to be more spread out. As the prior SD strengthens (like going from 50 to 1 then to 1/50 in our example) it exerts more influence on the posterior distribution so it becomes more resistant to noisy data. It will eventually become highly resistant to any deviation from the prior.
...
```{r, solution_03k}
exp(laplace_03_strong$log_evidence / laplace_06_strong$log_evidence)
exp(laplace_03_very_strong$log_evidence / laplace_06_very_strong$log_evidence)
```
It seems that the factor increases as the prior SD decreases. So, as the prior SD strengthens so does our evidence of which model is better.
...
```{r, solution_04c}
data_with_viz_grid <- data.frame(viz_grid)

X03_test <- model.matrix(~ (x1 + I(x1^2)) * (x2 + I(x2^2)), data = data_with_viz_grid)

# Summarize posterior predictions for the three prior specifications
num_samples <- 5000

# Summarize posterior predictions for the weak prior
post_pred_summary_viz_03_weak <- summarize_lm_pred_from_laplace(laplace_03_weak, X03_test, num_samples)

# Summarize posterior predictions for the strong prior
post_pred_summary_viz_03_strong <- summarize_lm_pred_from_laplace(laplace_03_strong, X03_test, num_samples)

# Summarize posterior predictions for the very strong prior
post_pred_summary_viz_03_very_strong <- summarize_lm_pred_from_laplace(laplace_03_very_strong, X03_test, num_samples)
```
...
post_pred_summary_viz_03_weak %>% 
  left_join(viz_grid %>% tibble::rowid_to_column("pred_id"),
            by = 'pred_id') %>%
  ggplot(aes(x = x1, y = mu_avg, ymin = y_lwr, ymax = y_upr)) +
  geom_line(color = "blue", linewidth = 1) +
  geom_ribbon(aes(ymax = y_upr, ymin = y_lwr), fill = "blue", alpha = 0.3) +
  labs(title = "Posterior Predictions (Weak Prior)",
       x = "x1",
       y = "Predicted Mean") +
  facet_wrap(~x2, scales = "free_x")
```
...
post_pred_summary_viz_03_strong %>% 
  ...
  geom_ribbon(aes(ymax = y_upr, ymin = y_lwr), fill = "red", alpha = 0.3) +
  labs(title = "Posterior Predictions (Strong Prior)",
       x = "x1",
       y = "Predicted Mean") +
  facet_wrap(~x2, scales = "free_x")
```
...
post_pred_summary_viz_03_very_strong %>% 
  ...
  geom_ribbon(aes(ymax = y_upr, ymin = y_lwr), fill = "forestgreen", alpha = 0.3) +
  labs(title = "Posterior Predictions (Very Strong Prior)",
       x = "x1",
       y = "Predicted Mean") +
  facet_wrap(~x2, scales = "free_x")
```
...
```{r, solution_04e}
X06_test <- model.matrix(~ splines::ns(x1, df = 12) * (x2 + I(x2^2) + I(x2^3) + 
    I(x2^4)), data = data_with_viz_grid)

#knots <- make_splines_training_knots(12, data_with_viz_grid, "x1")
#interior_knots <- knots$interior_knots
#boundary_knots <- knots$boundary_knots

#X06_test <- viz_grid %>%
#  mutate(splines = map(x1, ~splines::ns(., df = 12) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)))) %>%
#  unnest_wider(splines)

# Summarize posterior predictions for the three prior specifications
num_samples <- 5000

# Summarize posterior predictions for the weak prior
post_pred_summary_viz_06_weak <- summarize_lm_pred_from_laplace(laplace_06_weak, X06_test, num_samples)

# Summarize posterior predictions for the strong prior
post_pred_summary_viz_06_strong <- summarize_lm_pred_from_laplace(laplace_06_strong, X06_test, num_samples)

# Summarize posterior predictions for the very strong prior
post_pred_summary_viz_06_very_strong <- summarize_lm_pred_from_laplace(laplace_06_very_strong, X06_test, num_samples)
```
...
The behavior of the predictions is consistent with the behavior of the posterior coefficients. As the prior strengthens (SD decreases) there is less variability within the predictions and the confidence intervals become more narrow.
...
```{r, solution_05a}
X03_glmnet <- model.matrix(~ (x1 + I(x1^2)) * (x2 + I(x2^2)) - 1, data = data_with_viz_grid)
```

```{r, solution_05a_2}
X06_glmnet <- model.matrix(~ splines::ns(x1, df = 12) * (x2 + I(x2^2) + I(x2^3) + 
    I(x2^4)) - 1, data = data_with_viz_grid)
```
...
#lasso_03 <- glmnet(X03_glmnet, post_pred_summary_viz_03_strong$y_avg)
lasso_03 <- glmnet(X03_glmnet, pred_lm_03$pred)
```

```{r, solution_05b_2}
#lasso_06 <- glmnet(X06_glmnet, post_pred_summary_viz_06_strong$y_avg)
lasso_06 <- glmnet(X06_glmnet, pred_lm_06$pred)
```
...
lasso_03_cv_tune <- cv.glmnet(X03_glmnet, pred_lm_03$pred, alpha = 1)
```
...
lasso_06_cv_tune <- cv.glmnet(X06_glmnet, pred_lm_06$pred, alpha = 1)
```
...
# Number of coefficients remaining for Model 3
coefficients_remaining_03 <- coef(lasso_03_cv_tune, s = "lambda.min")
num_coeffs_remaining_03 <- sum(coefficients_remaining_03 != 0)
...
# Number of coefficients remaining for Model 6
coefficients_remaining_06 <- coef(lasso_06_cv_tune, s = "lambda.min")
num_coeffs_remaining_06 <- sum(coefficients_remaining_06 != 0)
```

```{r, solution_05e_3}
cat("Number of coefficients remaining for Model 3:", num_coeffs_remaining_03, "\n")
cat("Number of coefficients remaining for Model 6:", num_coeffs_remaining_06, "\n")
```
...
```{r, solution_05f}
coef(lasso_03_cv_tune, s = "lambda.min")
```

```{r, solution_05f_2}
coef(lasso_06_cv_tune, s = "lambda.min")
```

Yes, they are different. Are they supposed to be the same? Honestly I think I screwed this up somewhere.
